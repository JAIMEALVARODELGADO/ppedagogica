...
<form name='form1' method="post" action="pp_departamento11.php">
    <table width="50%">
        ...
        <th>Nombre</th>
        <?php
            $consulta="SELECT id_departamento,nombre FROM departamento ORDER BY nombre";
            $res=$link->query($consulta);
            if($res->num_rows<>0){
                while($row=$res->fetch_array()){
                    echo "<tr>";
                    echo "<td width='5%'><a href='#' onclick=editar('$row[id_departamento]') title='Editar' class='btnhref'><span class='icon-edit'></span></a></td>";
                    echo "<td width='5%'><a href='#' onclick=eliminar('$row[id_departamento]') title='Eliminar' class='btnhref'><span class='icon-trash Eliminar'></span></a></td>";
